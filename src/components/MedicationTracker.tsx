...
interface MedicationTrackerProps {
  ...
  onMedicationTaken: (index: number) => void;
}

const MedicationTracker: React.FC<MedicationTrackerProps> = ({ medications, onMedicationTaken }) => {
        ...
        <CardContent>
          ...
          <div className="grid gap-4">
            {medications.map((medication, index) => (
              <Card key={index} className={`transition-all duration-300 hover:shadow-lg ${medication.taken ? 'bg-green-50 border-green-200' : 'bg-white border-gray-200'}`}>
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      {!medication.taken && (
                        <Button 
                          onClick={() => onMedicationTaken(index)}
